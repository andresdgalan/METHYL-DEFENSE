...
# Definir umbral máximo de cobertura
...
# Calcular cuántos sitios tienen coverage > 1000 en al menos una muestra
n_sites_over_1000 <- sum(apply(meth_pre[, cov_cols], 1, function(x) any(x > 1000, na.rm = TRUE)))

# Mantener solo sitios con cobertura <= maximum_coverage en todas las muestras
CpG_meth_coverage <- CpG_meth_pre_filter[rowMeans(getData(CpG_meth_pre_filter)[, cov_cols] > maximum_coverage, na.rm = TRUE) == 0, ]
...
# --- 3. Filtrado por MEF (Minor Epiallele Frequency) ---
mef_threshold <- 0.01
meth <- getData(CpG_meth_filtered_polymorphic)

numCs <- as.matrix(dplyr::select(meth, starts_with("numC")))
numTs <- as.matrix(dplyr::select(meth, starts_with("numT")))

minor_counts <- pmin(numCs, numTs, na.rm = TRUE)
total_counts <- numCs + numTs

mef <- rowSums(minor_counts, na.rm = TRUE) / rowSums(total_counts, na.rm = TRUE)

keep_mef <- mef >= mef_threshold
...
We used "processBismarkAln" function of the methylKit R package to call all the cytosines in CpG position reported in the bam files with a minimum coverage of 5 reads. `r n_sites_over_1000` Positions with coverage above `r maximum_coverage` were removed, since such high values likely reflect artifacts from collapsed repetitive regions. From a total of **`r nrow(CpG_meth_pre_filter) `** positions, **`r nrow(CpG_meth_filtered_polymorphic) `** were polymorphic and therefore included in our SMP dataset. Additionally, SMPs with Minor Allele Frequency (MAF) ≤ `r mef_threshold * 100`% were removed as rare, uninformative, and potentially unreliable. Finally, we filtered the resulting **`r nrow(CpG_meth_filtered_mef) `** SMPs to keep only those present in at least `r threshold * 100`% of the individuals of every treatment (i.e., combinations of maternal environment and herbivory simulation treatments). After all the filters, we checked the missing data for every sample and the maximum was `r round(max(na_percent_df$pct_missing), 1)`%. Thus, we considered unnecessary to filter by sample, resulting in **`r nrow(CpG_meth_80_by_group) `** SMPs ready to be analysed.
