---
title: "DNA Methylation Analysis of A.barbata Samples using methylKit"
author: "Andrés Delgado"
date: "2025-08-29"
output: html_document
---

En este documento presento algunos resultados sobre los patrones de metilación de Avena bajo distintos tratamientos de herbivoría.

Para ahorrar tiempo y evitar errores, estoy depurando el flujo de trabajo (preprocesado, cálculo de porcentajes, modelos, visualizaciones, etc.) solo con Avena y solamente en contexto CpG. Estoy preparando este script de forma reproducible. Así, cuando quiera trabajar con otras especies u otros contextos como CHH y CHG, solamente será cuestión de usar el mismo script y sustituir una especie o un contexto por otro.
...
2.  Alineamiento de las lecturas de bisulfito de mis muestras tratadas con el pseudogenoma de referencia usando BISMARK en linux.
...
bam_dir <- "C:/Users/iglorena.IRNASE/Documents/andres/INTERTROPHIC/EPIGENETICA/NOVOGENE_DOWNLOAD/methylation_report_avena/andres_genome"
...
# avena_CpG_obj <- processBismarkAln(
#   bam.list,
#   sample.id = sample.id,
#   assembly = "mock",
#   treatment = treatment_multi,
#   mincov = 5,         # can be adjusted if neccesary
#   read.context = "CpG"     
# )
# 
# # Save the result for later use
# saveRDS(avena_CpG_obj, file = "myCpG.rds")
```

```{r CpG LOADING METHYLOBJECT, include=FALSE}
avena_CpG_obj <- readRDS(here("avena_pseudo", "avena_pseudo_CpG.rds"))
```
...
write.table(loadings_df, "avena_pseudo_cpg_loadings_with_positions.csv",
...
res_GUvsUC <- pairwise_diff(CpG_meth, g1 = 2, g2 = 1)
```
