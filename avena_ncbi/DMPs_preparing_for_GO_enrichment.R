...
setwd(here("avena_ncbi"))

# ----- Part A: combine contexts into DMPs_all_<comparison>.txt (as before) -----
comparisons <- c("GUvsGC", "GUvsUU", "UUvsUC")
contexts <- c("CpG", "CHG", "CHH")

# Helper function to generate contig_id if missing
generate_contig_id_if_needed <- function(df) {
  # detect existing columns
  contig_candidates <- c("contig_id", "contig", "seq_id", "ID")
  if (any(contig_candidates %in% names(df))) {
    return(df)  # nothing to do
  }
  
  # require chr + start
  if (!("chr" %in% names(df) && "start" %in% names(df))) {
    stop("Cannot generate contig_id: no contig-like column and missing chr/start columns.")
  }
  
  df$contig_id <- paste0(df$chr, "_", df$start)
  return(df)
}
...
for (comp in comparisons) {
  ...
  for (ctx in contexts) {
    ...
    # Ensure contig_id exists
    df <- generate_contig_id_if_needed(df)
...
for (f in all_dmp_files) {
  ...
  # Ensure contig_id exists (new feature)
  df <- generate_contig_id_if_needed(df)
  ...
  if (is.null(methdiff_col)) {
    ...
    write.table(data.frame(), file = sub("\\.txt$", "_ranked.txt", f),
  ...
  if (nrow(ranked_agg) > 0) {
    write.table(ranked_agg, file = ranked_out, sep = "\t",
                quote = FALSE, row.names = FALSE, col.names = FALSE)
    message("  Wrote ranked file: ", ranked_out, " (", nrow(ranked_agg), " IDs)")
  } else {
    write.table(data.frame(), file = ranked_out, sep = "\t",
                quote = FALSE, row.names = FALSE, col.names = FALSE)
