...
# Save the object in the project to open it from other computers
saveRDS(bam.list, file = "bam_list.rds")
...
```{r CpG LOADING METHYLOBJECT, include=FALSE}
avena_CpG_obj <- readRDS(here("avena_ncbi", "avena_ncbi_CpG.rds"))
```

```{r CpG FILTERING AND UNITING, include=FALSE}
# Filter out sample IDs ending with "_2.1"
keep_samples <- !grepl("_2$", sample.id)

# Apply filters
filtered_list <- avena_CpG_obj[keep_samples]
filtered_treatment <- avena_CpG_obj@treatment[keep_samples]

# Convert back to methylRawList (no sample.id argument needed)
avena_CpG_obj_filtered <- new("methylRawList", filtered_list, treatment = filtered_treatment)

# Número de CpGs en cada muestra
sapply(avena_CpG_obj_filtered, nrow)

# Unite all samples into one object to compare methylation across samples at common CpG sites
CpG_meth <- methylKit::unite(avena_CpG_obj_filtered)
```
