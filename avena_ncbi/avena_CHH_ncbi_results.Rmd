...
```{r CHH LOADING METHYLOBJECT, include=FALSE}
avena_CHH_obj <- readRDS(here("avena_ncbi", "avena_ncbi_CHH.rds"))
```

```{r CHH FILTERING AND UNITING, include=FALSE}
# Filter out sample IDs ending with "_2.1"
keep_samples <- !grepl("_2$", sample.id)

# Apply filters
filtered_list <- avena_CHH_obj[keep_samples]
filtered_treatment <- avena_CHH_obj@treatment[keep_samples]

# Convert back to methylRawList (no sample.id argument needed)
avena_CHH_obj_filtered <- new("methylRawList", filtered_list, treatment = filtered_treatment)

# Unite all samples into one object to compare methylation across samples at common CHH sites
CHH_meth <- methylKit::unite(avena_CHH_obj_filtered)
```

## CLUSTERING METHYLATION PROFILES

El primer paso en el análisis ha sido explorar visualmente las diferencias en los perfiles de metilación entre tratamientos. Para ello hemos utilizado los porcentajes de metilación de cada una de las posiciones CHH (posiciones del genoma con una Citosina seguida de dos bases distintas de Guanina). Hemos evaluado un total de **`r nrow(CHH_meth)`** posiciones compartidas por las **`r (ncol(CHH_meth) - 4)/3 `** muestras, separando por tratamientos para ver posibles agrupaciones. Aquí muestro un Correlograma, un Dendograma y un DAPC (Discriminant Analysis of Principal Components), que es un análisis multivariante usado en genética de poblaciones para describir y visualizar la estructura genética entre grupos.Utiliza el PCA y luego maximiza la separación entre grupos predefinidos (los tratamientos).

```{r CHH METHYLATION % MATRIX, include=FALSE}
CHH_meth_matrix <- as.matrix(percMethylation(CHH_meth))
# extract genomic positions
coords <- paste(getData(CHH_meth)$chr, getData(CHH_meth)$start, getData(CHH_meth)$strand, sep = "_")
